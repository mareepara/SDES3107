...
void setup(){
  size(600, 600, OPENGL);
  println ("changed translate diamater aka how much the image moves with the mouse");
}
...
background (250,247,151); 
  ...
  for (int gridY=0; gridY<width; gridY+=25) {
    for (int gridX=0; gridX<height; gridX+=25) {
      ...
      pushMatrix();
      translate(gridX, gridY, diameter*20);
      fill (133,47,247,80);
            rotate(PI/8);
            rect(25, 25, 50, 50);    //// also nice: ellipse(...)
      popMatrix(); 
...
//changed translate diamater aka how much the image moves with the mouse
